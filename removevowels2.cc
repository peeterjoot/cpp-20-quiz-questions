...
// sample solution was an in-place removal.  Here's one way to do that:
...
{
#if 0
   // fool compiler, if required.  can the optimizer precompute the whole program result?
   assert( argc == 1 );
   char * s = strdup( argv[1] );
   assert( s );

#else
   char s[] { "Hi there, how are \xFB\xE2you?" };
#endif
   removevowels( s );
   printf( "%s\n", s );
